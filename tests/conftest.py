...
def pytest_addoption(parser):
    ...
    # starmie config
    parser.addoption("--threshold", action="store", default="0.6")
    ...
    parser.addoption("--preprocessed_data", action="store", default="False")
    parser.addoption(
        "--text_format", action="store", default="numerical"
    )  # only support text as numerical or embedding vector

    parser.addoption("--target_query", action="store", default="all")

...
from datalake_vis.vis_instance.basic_vis_instance import (
    BasicIndVisInstance,
    BasicMergeVisInstance,
)
...
def test_basic_complete(vis_args):
    ...
    for query, matchings in tus_res.items():
        ...
        try:
            vis_ins = (
                BasicIndVisInstance(**args) if vis_args["vis_ins"] == "basic_ind" else BasicMergeVisInstance(**args)
            )
        ...
        for x in range(vis_ins.query_table.size()[1]):
            # if vis_ins.query_table[x].column_name != "rankmostimptchild_3@(categorical,False)":
            #     continue
            try:
                sub_start = time.time()
            ...
            for _, (x_series, y_series) in enumerate(y_to_series.values()):
                ...
                # for each viable ColVisPlan
                for _, plan in enumerate(y_vis_plans):
                    try:
                        sub_start = time.time()
                        plan.plot_data = vis_ins.compute_aggregate(
                            x_series, y_series, [plan.aggr], buckets, bucket_locator
                        )[plan.aggr]
                    ...
                    try:
                        sub_start = time.time()
                        cur_plan_util = plan.compute_utility(EMD.compute_score)
                        plan_util_time += time.time() - sub_start
                        plan_cnt += 1
                        plan_util += cur_plan_util
                    except Exception as e:
    ...
    vis_res_log.write(f"Utility sum of ColVisPlan per query: {str(col_vis_plan_util)}\n")
